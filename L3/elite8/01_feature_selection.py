...
# Load unified training dataset (replaces old long/rich split)
training_set_unified = pd.read_csv(config.get_training_dataset_path())
tournament_results = pd.read_csv(config.TOURNAMENT_RESULTS_FILE)

print(f"training_set_unified: {training_set_unified.shape[0]} rows, {training_set_unified.shape[1]} columns")
...
df_labeled = training_set_unified.merge(
    tournament_results[['Year', 'Index', 'tournamentSeed', 'tournamentOutcome']],
    on=['Year', 'Index'],
    how='left'
)
...
output_file = OUTPUT_DIR / 'labeled_training_unified.csv'
df_labeled.to_csv(output_file, index=False)

print(f"\nSaved labeled dataset to {OUTPUT_DIR}/")
...
feature_cols = [col for col in df_labeled.select_dtypes(include=[np.number]).columns 
               if col not in EXCLUDE_COLS]
...
# Save full list
high_corr_df.to_csv(OUTPUT_DIR / 'high_correlations_unified.csv', index=False)
print(f"  Saved full list to {OUTPUT_DIR}/high_correlations_unified.csv")
...
# Get numeric features (using config exclusion list)
numeric_features = [col for col in df_labeled.select_dtypes(include=[np.number]).columns 
                   if col not in EXCLUDE_COLS]
...
for feature in numeric_features:
    valid_mask = df_labeled[[feature, 'elite8_flag']].notna().all(axis=1)
    if valid_mask.sum() > 10:
        corr, _ = pearsonr(df_labeled.loc[valid_mask, feature], df_labeled.loc[valid_mask, 'elite8_flag'])
        correlations.append({
            'feature': feature,
...
# Save
corr_df.to_csv(OUTPUT_DIR / 'label_correlations_unified.csv', index=False)
print(f"  Saved full ranking to {OUTPUT_DIR}/label_correlations_unified.csv")
...
if vif_data:
    ...
    vif_df.to_csv(OUTPUT_DIR / 'vif_analysis_unified.csv', index=False)
    print(f"\n  Saved full VIF analysis to {OUTPUT_DIR}/vif_analysis_unified.csv")
...
high_corr_df = pd.read_csv(OUTPUT_DIR / 'high_correlations_unified.csv')
...
label_corr = pd.read_csv(OUTPUT_DIR / 'label_correlations_unified.csv')
...
# Load VIF
try:
    vif_df = pd.read_csv(OUTPUT_DIR / 'vif_analysis_unified.csv')
...
drop_df.to_csv(OUTPUT_DIR / 'drop_recommendations_unified.csv', index=False)
...
# IMPORTANT: PowerRank should NOT be dropped if present (new signal)
powerrank_dropped = False
for pr_name in ['PowerRank', 'powerrank']:
    if pr_name in features_to_drop:
        powerrank_dropped = True
        break

if powerrank_dropped:
    print(f"\n  ⚠️  PowerRank was marked for removal but should be reviewed")
    print(f"  PowerRank is a continuous RATING (not rank) and may provide unique signal")
    print(f"  Consider keeping PowerRank unless correlation analysis shows it's redundant")

reduced_features_df = pd.DataFrame({'feature': features_to_keep})
reduced_features_df.to_csv(OUTPUT_DIR / 'reduced_features_unified.csv', index=False)
...
print(f"  Saved reduced feature list to {OUTPUT_DIR}/reduced_features_unified.csv")
...
print("\nOUTPUTS GENERATED:")
print(f"  {OUTPUT_DIR}/labeled_training_unified.csv - Training data with elite8_flag label")
print(f"  {OUTPUT_DIR}/high_correlations_unified.csv - Highly correlated feature pairs")
print(f"  {OUTPUT_DIR}/label_correlations_unified.csv - Features ranked by label correlation")
print(f"  {OUTPUT_DIR}/vif_analysis_unified.csv - Variance inflation factors")
print(f"  {OUTPUT_DIR}/drop_recommendations_unified.csv - Recommended features to drop")
print(f"  {OUTPUT_DIR}/reduced_features_unified.csv - Recommended features to keep")
...
print("\n✓ UNIFIED DATASET:")
print("  - Single training set (2008-2025, 1,147 teams)")
print("  - Sources: bartTorvik, kenPom, BPI, Massey, PowerRank")
print("  - PowerRank: Continuous rating, NOT normalized to 0-364")
print("  - Replaces old LONG/RICH split architecture")

print("\nNEXT STEPS:")
print("  1. Review correlation heatmaps and drop recommendations")
print("  2. Decide which features to keep for modeling")
print("  3. Proceed to 02_ensemble_models.py")
