...
│   ├── data/
│   │   ├── trainingData/
│   │   │   ├── training_set_long.csv      (17 years, rich features)
│   │   │   └── training_set_rich.csv      (subset with complete data)
│   │   └── predictionData/
...
Output: Chalk score → Tournament type → Expected performance
  - CHALK (score ≥5): 60-80% probability, expect 4-6/8 correct
...
FADE CANDIDATES (overseeded - metrics skeptical):
  - Houston:  56.8% WITH seeds → 52.9% metrics (-3.9%)
  - Purdue:   56.9% WITH seeds → 53.6% metrics (-3.3%)
  - Florida:  57.7% WITH seeds → 54.8% metrics (-2.9%)
  - Illinois: 55.1% WITH seeds → 52.2% metrics (-2.9%)
...
CONSERVATIVE POOL STRATEGY (Friends, Small Pools):
  • Trust top 4-5 consensus picks
  • Pick teams >50% for Elite 8
  • Avoid risky differentiation
  • Expected finish: Top 15-25%

COMPETITIVE POOL STRATEGY (ESPN, Large Pools):
  • Trust top 4 as locks: Arizona, UConn, Michigan, Duke
  • FADE overseeded: Houston, Purdue, Florida (everyone picks them)
  • Add 1-2 variance plays from 40-50% range
  • Expected finish: Top 20-30% (hard to differentiate in chalk years)

CALCUTTA AUCTION STRATEGY:
  • TARGET: Top 4 consensus (expensive but justified)
  • AVOID: Houston, Purdue, Florida, Illinois (overvalued by seeds)
  • Budget allocation: 60% top-4, 30% middle tier, 10% fliers
  • In chalk years, favorites win - pay up for quality
